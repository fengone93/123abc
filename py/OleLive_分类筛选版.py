...
说明：欧乐接口可能随时调整。权限值按照当前列表接口的三态参数整理为：
3=全部、1=免费、2=会员。如站点改变该映射，只需修改 ACCESS_VALUES。
"""
...
class Spider(Spider):
    ...
    def _filter_for_type(self, item):
        ...
        return [
            ...
            },
            {
                "key": "letter",
                "name": "字母",
                "value": self._values(letters, "全部", "0"),
            },
            {
                "key": "access",
                "name": "权限",
                "value": [
                    {"n": "全部", "v": self.ACCESS_VALUES["all"]},
                    {"n": "免费", "v": self.ACCESS_VALUES["free"]},
                    {"n": "会员", "v": self.ACCESS_VALUES["vip"]},
                ],
            },
            {
                "key": "sort",
                "name": "排序",
                "value": [
                    {"n": "按最新", "v": self.SORT_VALUES["update"]},
                    {"n": "按添加", "v": self.SORT_VALUES["add"]},
                    {"n": "按最热", "v": self.SORT_VALUES["hot"]},
                    {"n": "按评分", "v": self.SORT_VALUES["score"]},
                ],
            },
        ]
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        # 当前公开接口路径：固定媒体类型/权限/字母/地区/主分类/子类型/年份/排序/页码/条数。
        path = (
